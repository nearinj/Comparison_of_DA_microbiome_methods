...
eig_filt_plot <- ~barplot(eig_percent, xlab='Component', ylab="Percent Explained", ylim = c(0,30))
...
PC4_set <- paste("PC4 (", as.character(round(eig_percent[4],3)), "%)", sep="")
...
panelA <- plot_grid(eig_filt_plot, plot12, plot13, plot14)
panelA
...
eig_percent_unfilt_plot <- ~barplot(eig_percent_unfilt, xlab='Component', ylab="Percent Explained", ylim = c(0,30))
...
PC4_set_unfilt <- paste("PC4 (", as.character(round(eig_percent_unfilt[4],3)), "%)", sep="")
...
Sup_figB <- plot_grid(eig_percent_unfilt_plot, plot12_unfilt, plot13_unfilt, plot14_unfilt)
Sup_figB

```

```{r}
Figure3 <- plot_grid(plot12_unfilt, plot12, labels = c("A", "B"))
Figure3
```