...
Office_per_sig <- get_percent_sig(Filtered_FD_data[["Office"]], 185, 185)

ArcticFreshWaters_per_sig <- get_percent_sig(Filtered_FD_data[["ArcticFreshWaters"]], 344, 343)

Blueberry_per_sig <- get_percent_sig(Filtered_FD_data[["Blueberry"]], 3250, 3245)

ob_goodrich_per_sig <- get_percent_sig(Filtered_FD_data[["ob_goodrich"]], 4287, 4287)

hiv_noguerajulian_per_sig <- get_percent_sig(Filtered_FD_data[["hiv_noguerajulian"]], 3092, 2842)

Ji_WTP_DS_per_sig <- get_percent_sig(Filtered_FD_data[["Ji_WTP_DS"]], 242, 242)

cdi_schubert_per_sig <- get_percent_sig(Filtered_FD_data[["cdi_schubert"]], 599, 599)

sw_sed_detender_per_sig <- get_percent_sig(Filtered_FD_data[["sw_sed_detender"]], 1553, 1998)
...
names(Unfiltered_FD_data_ext)

### something odd going on with the ext data...
...
ArcticFreshWaters_per_sig_un <- get_percent_sig(ArcticFreshWater_FD_data_comb, 65597, 35419)

Blueberry_per_sig_un <- get_percent_sig(Blueberry_FD_data_comb, 7827, 8012)

ob_goodrich_per_sig_un <- get_percent_sig(ob_goodrich_data_comb, 55929, 42602)

hiv_noguerajulian_per_sig_un <- get_percent_sig(hiv_noguerajulian_data_comb, 14584, 8967)

Ji_WTP_DS_per_sig_un <- get_percent_sig(Ji_WTP_DS_data_comb, 2163, 1831)

Office_per_sig_un <- get_percent_sig(Office_data_comb, 50238, 34614)

cdi_schubert_per_sig_un <- get_percent_sig(cdi_schubert_data_comb, 3039, 2922)

sw_sed_detender_per_sig_un <- get_percent_sig(sw_sed_detender_data_comb, 5893, 2648)
...
ggsave(filename=paste(display_items_out, "Main_figures", "Figure4.pdf", sep="/"),
       plot = Figure4_plot, width = 6, height=7, units="in", dpi=600)

ggsave(filename=paste(display_items_out, "Main_figures", "Figure4.png", sep="/"),
       plot = Figure4_plot, width = 6, height=7, units="in", dpi=100)
