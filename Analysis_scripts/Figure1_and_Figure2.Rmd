...
Metadata_renames <- c(log_N="log(Sample size)", R.squared="Aitchson's Dist. Effect Size", log_R.squared="log(Aitch. dist. effect size)", Sparsity="Sparsity", Richness="Richness", log_Depth="log(Max read depth)", log_Depth_range="log(Read depth range)", CoV_Depth="Read depth variation")
```
...
for(i in 1:length(colnames(filt_sig_percent_fix))){
  ...
  #go through a total of 7 different dataset characteristics
  for(j in 1:8){
...
for(i in 1:length(colnames(unfilt_sig_percent_fix))){
  ...
  for(j in 1:8){
    ...
    temp_pvals_unfilt[[j]] <- cor.test(unfilt_sig_percent_fix[,i], fixed_hackathon_metadata_unfilt[,(j+29)], method="spearman", exact=F)$p.value
    temp_rhos_unfilt[[j]] <- cor.test(unfilt_sig_percent_fix[,i], fixed_hackathon_metadata_unfilt[,(j+29)], method="spearman", exact=F)$estimate
  }
