...
Metadata_renames <- c(log_N="log(Sample size)", R.squared="Aitchson's Dist. Effect Size", log_R.squared="log(Aitch. dist. effect size)", Sparsity="Sparsity", Richness="Richness",
                      log_Depth="log(Max read depth)", log_Depth_range="log(Read depth range)", CoV_Depth="Read depth variation", percent_chimera="% chimeras", filtered_out_percent = "% below prev. cut-off")
```
...
Calc_richess_diff_between_filt_and_unfilt  <- function(filt_tab, unfilt_tab){
...
unfilt_percent_under_10percent.prevalence <- Calc_richess_diff_between_filt_and_unfilt(filt_tab = filt_study_tab,
...
# Load in % chimera results
percent_chimera <- read.table("/home/gavin/github_repos/hackathon/Comparison_of_DA_microbiome_methods/Misc_datafiles/uchime_ref_chimera_parsing/uchime_ref_chimera_percents.tsv",
                              header = TRUE, sep = "\t", row.names = 1)

fixed_hackathon_metadata_filt$percent_chimera <- percent_chimera[rownames(fixed_hackathon_metadata_filt), "filt_nonrare"]
...
filt_ASV_nums <- pheatmap(t(Alpha_order_filt),
                         ...
                         annotation_row=fixed_hackathon_metadata_filt[, c("log(Sample size)", "log(Aitch. dist. effect size)", 
                                                                     ...
                                                                     "log(Read depth range)", "% chimeras"), drop=FALSE],
...
# Get percent chimeras
fixed_hackathon_metadata_unfilt$percent_chimera <- percent_chimera[rownames(fixed_hackathon_metadata_unfilt), "unfilt_nonrare"]
...
unfilt_ASV_nums <- pheatmap(t(Alpha_order_unfilt),
                           ...
                           annotation_row=fixed_hackathon_metadata_unfilt[, c("log(Sample size)", "log(Aitch. dist. effect size)", 
                                                                       "Sparsity", "Richness", "Read depth variation", 
                                                                       "log(Read depth range)", "% chimeras", "% below prev. cut-off"), drop=FALSE],
                           annotation_legend=FALSE,
                           treeheight_col = 0,
                           cluster_cols = FALSE,
                           cluster_rows = TRUE,
...
colnames(filt_rhos_df) <- colnames(fixed_hackathon_metadata_filt)[30:38]
colnames(filt_pvals_df) <- colnames(fixed_hackathon_metadata_filt)[30:38]
...
colnames(unfilt_rhos_df) <- colnames(fixed_hackathon_metadata_unfilt)[30:39]
colnames(unfilt_pvals_df) <- colnames(fixed_hackathon_metadata_unfilt)[30:39]
